...
test("safeTemplateFileName generates stable safe JSON filenames", () => {
  ...
  assert.equal(safeTemplateFileName("../../../"), "template.json");
  assert.equal(safeTemplateFileName("My_Template--Draft!!.json"), "my_template-draftjson.json");
});
...
test("sessionAssetPath allows normalized contained asset paths", () => {
  const root = path.join(tmpdir(), "htmlprinter-root");

  assert.equal(
    sessionAssetPath(root, "sample", "assets/shot..v2.svg"),
    path.resolve(root, "imports", "sample", "assets", "shot..v2.svg")
  );
  assert.equal(
    sessionAssetPath(root, "sample", "assets/../assets/shot.svg"),
    path.resolve(root, "imports", "sample", "assets", "shot.svg")
  );
});

test("sessionAssetPath rejects session and asset traversal", () => {
  const root = path.join(tmpdir(), "htmlprinter-root");

  assert.throws(() => sessionAssetPath(root, "../sample", "assets/shot.svg"), /非法 session 路径/);
  assert.throws(() => sessionAssetPath(root, "sample", "../secret.png"), /非法图片路径/);
  assert.throws(() => sessionAssetPath(root, "sample", "assets/../../secret.png"), /非法图片路径/);
  assert.throws(() => sessionAssetPath(root, "sample", path.resolve(root, "outside.svg")), /非法图片路径/);
});
...
test("repository rejects template path traversal", async () => {
  ...
  assert.throws(() => repo.resolveAsset("sample", "../secret.svg"), /非法图片路径/);
