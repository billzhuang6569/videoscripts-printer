...
function isPrinterRoot(dir) {
  return (
    existsSync(path.join(dir, "src", "server", "validation.mjs")) &&
    existsSync(path.join(dir, "src", "server", "server.mjs")) &&
    existsSync(path.join(dir, "public", "index.html"))
  );
}

function resolvePrinterRoot() {
  const explicitRoot = argValue("--root") || process.env.HTMLPRINTER_ROOT || projectRoot;
  const root = path.resolve(explicitRoot);
  if (!isPrinterRoot(root)) {
    fail(`HTML printer root not found: ${root}\nRun scripts/ensure-printer.mjs first, then pass its "root" value with --root.`);
  }
  return root;
}

async function loadValidator(rootDir) {
  const validatorPath = path.join(rootDir, "src", "server", "validation.mjs");
...
async function main() {
  ...
  const rootDir = resolvePrinterRoot();
  const outputRoot = path.resolve(argValue("--output-root") || rootDir);
  const { validateSessionData } = await loadValidator(rootDir);
  ...
  const sessionDir = path.join(outputRoot, "imports", sessionId);
